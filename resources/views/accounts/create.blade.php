...
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.finance.add_account') }}</h1>
        </div>
            ...
            <form method="POST" action="{{ route('accounts.store') }}">
                ...
                <div class="space-y-4 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="account_number" value="{{ old('account_number') }}" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.finance.opening_balance') }} *</label>
                        <input type="number" name="opening_balance" value="{{ old('opening_balance', 0) }}" step="0.01" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                    </div>
                    <div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('accounts.index') }}" class="px-6 py-2.5 rounded-xl glass-solid border border-white/20 text-white text-sm hover:bg-white/20">{{ __('app.finance.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
