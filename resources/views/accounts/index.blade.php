...
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-6xl mx-auto">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.finance.accounts') }}</h1>
            <a href="{{ route('accounts.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#83b735] text-white font-medium text-sm hover:bg-[#6f9d2d] transition-all">
                {{ __('app.finance.add_account') }}
            </a>
        </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-white/20 bg-white/5">
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold text-white/90 uppercase">{{ __('app.finance.account_number') }}</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-white/10 hover:bg-white/5">
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('accounts.show', $a) }}" class="px-3 py-1.5 rounded-lg bg-white/10 text-white/90 text-sm hover:bg-[#83b735]/20">{{ __('app.finance.view') }}</a>
                                <a href="{{ route('accounts.edit', $a) }}" class="px-3 py-1.5 rounded-lg bg-white/10 text-white/90 text-sm hover:bg-[#83b735]/20">{{ __('app.finance.edit') }}</a>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="5" class="px-4 py-12 text-center text-white/60">{{ __('app.finance.no_accounts') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
