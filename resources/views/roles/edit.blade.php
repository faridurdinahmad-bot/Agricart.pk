...
@section('title', __('app.roles.edit_role') . ' - Agricart ERP')
...
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.roles.edit_role') }}</h1>
        </div>
        ...
        <div class="backdrop-blur-xl glass-panel border border-white/25 rounded-2xl p-6 sm:p-8">
            ...
            <form method="POST" action="{{ route('roles.update', $role) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/50 focus:ring-2 focus:ring-[#83b735] focus:border-transparent">
                </div>
                <div>
                    <label for="description" class="block text-sm font-medium text-white/90 mb-2">{{ __('app.roles.description') }}</label>
                    <input type="text" name="description" id="description" value="{{ old('description', $role->description) }}" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/50 focus:ring-2 focus:ring-[#83b735] focus:border-transparent">
                </div>
                <div>
                        ...
                        <div class="rounded-xl bg-white/5 border border-white/10 p-4">
                            ...
                            <div class="flex flex-wrap gap-3">
                                ...
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $permKey }}" {{ in_array($permKey, old('permissions', $role->permissions ?? [])) ? 'checked' : '' }} class="w-4 h-4 rounded border-white/30 bg-white/20 text-[#83b735] focus:ring-[#83b735]">
