...
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.inventory.add_unit') }}</h1>
        </div>
        ...
        <div class="backdrop-blur-xl glass-panel border border-white/25 rounded-2xl p-6">
            ...
            <form method="POST" action="{{ route('units.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Kilogram" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                </div>
                <div>
                    <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.inventory.symbol') }}</label>
                    <input type="text" name="symbol" value="{{ old('symbol') }}" placeholder="e.g. kg" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                </div>
                <div>
