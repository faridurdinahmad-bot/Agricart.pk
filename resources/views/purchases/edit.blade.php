...
<div class="page-container">
    <div class="max-w-4xl mx-auto">
        <x-page-back :href="route('purchases.show', $purchase)" :title="__('app.purchase.edit_order') . ' - ' . $purchase->reference_number" />
        ...
        <x-card>
            ...
            <form method="POST" action="{{ route('purchases.update', $purchase) }}" x-data="{ items: {{ json_encode($purchase->items->map(fn($i) => ['product_id' => (string)$i->product_id, 'quantity' => $i->quantity, 'rate' => $i->rate])->values()) }} }">
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <x-input type="date" name="date" :label="__('app.purchase.date') . ' *'" :value="old('date', $purchase->date->format('Y-m-d'))" required />
                </div>
                <div class="mb-6">
                    <x-input type="date" name="expected_date" :label="__('app.purchase.expected_date')" :value="old('expected_date', $purchase->expected_date?->format('Y-m-d'))" />
                </div>
                <div class="mb-6">
                    <x-input type="textarea" name="notes" :label="__('app.purchase.notes')" :rows="2">{{ old('notes', $purchase->notes) }}</x-input>
                </div>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <x-button :href="route('purchases.show', $purchase)" variant="secondary">{{ __('app.purchase.cancel') }}</x-button>
                </div>
            </form>
        </x-card>
    </div>
