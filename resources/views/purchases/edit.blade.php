...
@section('title', __('app.purchase.edit_order') . ' - ' . $purchase->reference_number . ' - Agricart ERP')

@section('content')
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('purchases.show', $purchase) }}" class="p-2 rounded-xl glass-solid border border-white/20 text-white/90 hover:bg-white/20 transition-all">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.purchase.edit_order') }} - {{ $purchase->reference_number }}</h1>
        </div>
        ...
        <div class="backdrop-blur-xl glass-panel border border-white/25 rounded-2xl p-6 sm:p-8">
            ...
            <form method="POST" action="{{ route('purchases.update', $purchase) }}" x-data="{ items: {{ json_encode($purchase->items->map(fn($i) => ['product_id' => (string)$i->product_id, 'quantity' => $i->quantity, 'rate' => $i->rate])->values()) }} }">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', $purchase->date->format('Y-m-d')) }}" required class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                    </div>
                </div>
                <div class="mb-6">
                    <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.purchase.expected_date') }}</label>
                    <input type="date" name="expected_date" value="{{ old('expected_date', $purchase->expected_date?->format('Y-m-d')) }}" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                </div>
                <div class="mb-6">
                    <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.purchase.notes') }}</label>
                    <textarea name="notes" rows="2" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">{{ old('notes', $purchase->notes) }}</textarea>
                </div>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-[#83b735] text-white font-semibold hover:bg-[#6f9d2d]">{{ __('app.purchase.update') }}</button>
                    <a href="{{ route('purchases.show', $purchase) }}" class="px-6 py-2.5 rounded-xl glass-solid border border-white/20 text-white text-sm hover:bg-white/20">{{ __('app.purchase.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
