...
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('purchase-returns.show', $purchaseReturn) }}" class="p-2 rounded-xl glass-solid border border-white/20 text-white/90 hover:bg-white/20 transition-all">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.purchase.edit_return') }} - {{ $purchaseReturn->reference_number }}</h1>
        </div>
            ...
            <form method="POST" action="{{ route('purchase-returns.update', $purchaseReturn) }}" x-data="{ items: {{ json_encode($purchaseReturn->items->map(fn($i) => ['product_id' => (string)$i->product_id, 'quantity' => $i->quantity, 'rate' => $i->rate])->values()) }} }">
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', $purchaseReturn->date->format('Y-m-d')) }}" required class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                    </div>
                </div>
                <div class="mb-6">
                    <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.purchase.reason') }}</label>
                    <textarea name="reason" rows="2" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">{{ old('reason', $purchaseReturn->reason) }}</textarea>
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('purchase-returns.show', $purchaseReturn) }}" class="px-6 py-2.5 rounded-xl glass-solid border border-white/20 text-white text-sm hover:bg-white/20">{{ __('app.purchase.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
