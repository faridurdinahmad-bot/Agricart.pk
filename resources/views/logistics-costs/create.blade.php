...
@section('title', __('app.logistics.add_cost') . ' - Agricart ERP')
...
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.logistics.add_cost') }}</h1>
        </div>
        ...
        <div class="backdrop-blur-xl glass-panel border border-white/25 rounded-2xl p-6 sm:p-8">
            ...
            <form method="POST" action="{{ route('logistics-costs.store') }}" class="space-y-5">
                @csrf
                <div>
                    ...
                    <select name="shipment_id" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                        ...
                        <option value="{{ $s->id }}" {{ old('shipment_id') == $s->id ? 'selected' : '' }}>#{{ $s->id }} {{ $s->tracking_number ? '- ' . $s->tracking_number : '' }} {{ $s->sale ? '(Sale #' . $s->sale->id . ')' : '' }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="category" value="{{ old('category') }}" placeholder="e.g. Shipping, Fuel, Maintenance" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/50">
                </div>
                <div>
                    <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.logistics.amount') }} *</label>
                    <input type="number" name="amount" value="{{ old('amount') }}" step="0.01" min="0.01" required class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/50">
                </div>
                <div>
                    <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.logistics.date') }} *</label>
                    <input type="date" name="date" value="{{ old('date', now()->format('Y-m-d')) }}" required class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                </div>
                <div>
                    <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.logistics.description') }}</label>
                    <textarea name="description" rows="3" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/50">{{ old('description') }}</textarea>
                </div>
                <div class="flex gap-3 pt-2">
                    ...
                    <a href="{{ route('logistics-costs.index') }}" class="px-6 py-2.5 rounded-xl glass-solid border border-white/20 text-white text-sm hover:bg-white/20 transition-all">{{ __('app.logistics.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
