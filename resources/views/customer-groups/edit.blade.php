        ...
        <div class="backdrop-blur-xl glass-panel border border-white/25 rounded-2xl p-6">
            ...
            <form method="POST" action="{{ route('customer-groups.update', $customerGroup) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $customerGroup->name) }}" required class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                </div>
                <div>
                    <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.contacts.description') }}</label>
                    <input type="text" name="description" value="{{ old('description', $customerGroup->description) }}" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.contacts.discount') }} (%)</label>
                        <input type="number" name="discount_percent" value="{{ old('discount_percent', $customerGroup->discount_percent) }}" step="0.01" min="0" max="100" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-white/90 mb-2">{{ __('app.contacts.price_type') }}</label>
                        <input type="text" name="price_type" value="{{ old('price_type', $customerGroup->price_type) }}" class="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white">
                    </div>
                </div>
