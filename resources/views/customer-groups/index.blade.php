...
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-4xl mx-auto">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.contacts.customer_groups') }}</h1>
            <a href="{{ route('customer-groups.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#83b735] text-white font-medium text-sm hover:bg-[#6f9d2d] transition-all">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-white/20 bg-white/5">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-bold text-white/90 uppercase">{{ __('app.contacts.discount') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-bold text-white/90 uppercase">{{ __('app.contacts.customers_count') }}</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-white/10 hover:bg-white/5">
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('customer-groups.edit', $g) }}" class="px-3 py-1.5 rounded-lg bg-white/10 text-white/90 text-sm hover:bg-[#83b735]/20">{{ __('app.contacts.edit') }}</a>
                            <form method="POST" action="{{ route('customer-groups.destroy', $g) }}" class="inline" onsubmit="return confirm('{{ __('app.contacts.delete_confirm') }}');">@csrf @method('DELETE')<button type="submit" class="px-3 py-1.5 rounded-lg bg-white/10 text-red-400 text-sm hover:bg-red-500/20">{{ __('app.contacts.delete') }}</button></form>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="5" class="px-4 py-12 text-center text-white/60">{{ __('app.contacts.no_customer_groups') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
