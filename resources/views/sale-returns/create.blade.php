...
<div class="px-3 sm:px-4 md:px-5 py-4 sm:py-5">
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-white">{{ __('app.sale.add_return') }}</h1>
        </div>
        ...
        <div class="backdrop-blur-xl glass-panel border border-white/25 rounded-2xl p-6 sm:p-8">
            ...
            <form method="POST" action="{{ route('sale-returns.store') }}" x-data="{ items: {{ json_encode($initialItems) }} }">
                ...
                <div class="flex gap-3">
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-[#83b735] text-white font-semibold hover:bg-[#6f9d2d]">{{ __('app.sale.save') }}</button>
                    <a href="{{ route('sale-returns.index') }}" class="px-6 py-2.5 rounded-xl glass-solid border border-white/20 text-white text-sm hover:bg-white/20">{{ __('app.sale.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
